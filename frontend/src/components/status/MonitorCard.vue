<template>
  <div class="glass-card rounded-2xl px-6 pt-5 pb-4 status-line cursor-default group"
    ...
    <div class="flex items-center justify-between gap-3 mb-2 monitor-row1">
      ...
      <div class="flex items-center gap-2.5 shrink-0 monitor-meta">
        <div v-if="monitor.latency != null && !monitor.paused" class="latency-badge flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-mono font-medium border cursor-default"
        ...
        <div v-if="monitor.uptime_24h != null && !monitor.paused" class="px-2.5 py-1 rounded-lg text-xs font-mono font-bold border"
        ...
        <span class="hidden sm:inline px-2.5 py-1 rounded-lg text-[11px] font-bold font-mono tracking-[0.1em] border transition-all duration-300"
    ...
    <!-- 第二行：URL + SSL + 迷你折线图 -->
    <div class="flex items-center justify-between gap-4 mb-3 pl-[26px] monitor-row2">
      <div class="flex items-center gap-3 min-w-0 flex-wrap">
        <a :href="monitor.url" target="_blank" rel="noopener" class="text-[13px] font-mono text-slate-400 dark:text-slate-500 hover:text-emerald-500 dark:hover:text-emerald-400 transition-colors truncate max-w-[320px] cursor-pointer flex items-center gap-1.5 group/link">
          {{ monitor.url }}
        ...
        <div v-if="monitor.cert_expiry" class="hidden sm:flex items-center gap-1 px-2 py-0.5 rounded text-[11px] font-mono" :class="getExpiryClass(monitor.cert_expiry)">
        ...
        <span class="text-[11px] font-mono text-slate-400 dark:text-slate-600">{{ formatDate(monitor.last_check) }}</span>
      </div>
      <!-- 迷你延迟折线 -->
      <div v-if="monitor.recent_latencies && monitor.recent_latencies.length > 2 && !monitor.paused" class="sparkline-wrap hidden sm:block shrink-0">
        <svg class="w-[120px] h-[28px]" :class="monitor.status === 'DOWN' ? 'text-red-500' : 'text-emerald-500'" viewBox="0 0 120 28" preserveAspectRatio="none">
